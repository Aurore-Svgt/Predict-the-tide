...
if __name__ == '__main__':
    ...
    for id in ['1','2']:
        ...
        model  = Pipeline([('pca', PCA(n_components=200)),
        ('KNN',KNeighborsRegressor(n_neighbors=100,weights='uniform'))
        #('XGB', MultiOutputRegressor( XGBRegressor(reg_alpha = 0.5,reg_lambda=3,gamma = 10,max_depth=3,min_child_weight = 30)) )
         ])
        model.fit(X_train[id],y_train[id])
        ## save model
        pickle.dump(model,open( "KNN"+id+".pkl", "wb" ))
